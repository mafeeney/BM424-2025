```{r data4I2, include=FALSE}

library(ggplot2)
library(knitr)

outbreak_deaths3I2 <- 29
outbreak_cases3I2 <- 525
outbreak_icu3I2 <- 12

outbreak_deaths4I2 <- 39
outbreak_cases4I2 <- 365
outbreak_icu4I2 <- 10
...
* Based on collated hospital reports and death certificates, it is believed that there have been `r outbreak_deaths4I2` deaths due to `r dis_name` in `r city_name` thus far

* There are currently `r outbreak_cases4I2` patients in the `r hosp_name` with confirmed `r dis_name`, or symptoms consistent with `r dis_name`

* There are currently `r outbreak_icu4I2`  patients currently in the ICU with confirmed `r dis_name`

* Genomic epidemiology of all samples isolated from patients indicates that they come from the same source.

* An antibiotic stewardship programme has been approved by the local NHS board and has been implemented.

* A medical supply provision and tracking system has been designed and implemented.

* A proposal for use of new CRISPR technologies in diagnosis has been submitted to the local NHS board. 