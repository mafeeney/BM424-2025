...
```{r A1, echo=FALSE, fig.cap="`r city_name` demographics (Source: Office for National Statistics)"}
...
ggplot(data = population, aes(x=PopYears, y=Pop)) + 
  geom_point() +
  geom_line() +
  labs(y = "Number of `r city_name` residents (estimated)", x = "Year") +
...
ggplot(data = birthdeath, aes(x=DemogYears, y=Num, group=Demog, colour=Demog)) + 
  geom_point() +
  geom_line() +
  labs(y = "Number of births and deaths in `r city_name` (estimated)", x = "Year", colour = "Demographics") +
...
```{r A2, fig.cap="Voting intention by `r city_name` residents (Source: YouGov polling)", echo=FALSE}
