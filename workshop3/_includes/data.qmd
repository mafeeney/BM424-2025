...
* Based on collated hospital reports and death certificates, it is believed that there have been `r outbreak_deaths` in `r city_name` thus far

* There are currently `r outbreak_cases` patients in the `r hospital_name` (with confirmed `r dis_name`, or symptoms consistent with `r dis_name`
...
```{r A1, echo=FALSE, fig.cap="Inverkeld demographics (Source: Office for National Statistics)"}
